...
function packet(m){const p=m.payload||{};if(m.type==='welcome'){S.id=p.id;auth();return;}if(m.type==='auth_ok'){S.authed=true;if(p.token)localStorage.setItem(TOKEN,p.token);window.ThinUI?.renderCharacter(p.character);window.ThinUI?.connection(true,true);startAutoLoop();return;}if(m.type==='snapshot'){S.snapshot=p;window.MMO149Visuals?.onSnapshot?.(p);window.ThinUI?.renderCharacter(p.self);window.ThinUI?.renderWorld(p.world);window.ThinUI?.appendEvent(p.event);return;}if(m.type==='presence'){S.online=p.players||[];window.ThinUI?.renderPresence(S.online);return;}if(m.type==='world_chat'){window.ThinUI?.appendChat(p);return;}if(m.type==='error'){console.warn('[MMO149]',p.message||'server error');return;}}

...
function packet(m){const p=m.payload||{};if(m.type==='welcome'){S.id=p.id;auth();return;}if(m.type==='auth_ok'){S.authed=true;if(p.token)localStorage.setItem(TOKEN,p.token);window.ThinUI?.renderCharacter(p.character);window.ThinUI?.connection(true,true);return;}if(m.type==='snapshot'){S.snapshot=p;window.ThinUI?.renderCharacter(p.self);window.ThinUI?.renderWorld(p.world);window.ThinUI?.appendEvent(p.event);return;}if(m.type==='presence'){S.online=p.players||[];window.ThinUI?.renderPresence(S.online);return;}if(m.type==='world_chat'){window.ThinUI?.appendChat(p);return;}if(m.type==='error'){console.warn('[MMO149]',p.message||'server error');return;}}
...
window.MMO149Realtime={connect,send,isConnected:()=>S.connected,isAuthoritative:()=>S.authed,onlinePlayers:()=>S.online.slice(),snapshot:()=>S.snapshot,getServerUrl:()=>SERVER_URL,rollStats:()=>window.ThinUI?.showRoll({rollId:'server',stats:{str:'?',dex:'?',con:'?',int:'?',wis:'?',cha:'?'}}),createCharacter:(name,cls)=>send('auth',{token:'',name,cls}),changeMap:map=>serverOnly('map_change',{map}),move:(x,y)=>serverOnly('move',{x,y}),attack:uid=>serverOnly('attack',{uid}),equip:uid=>serverOnly('equip',{uid}),unequip:slot=>serverOnly('unequip',{slot}),useItem:uid=>serverOnly('use_item',{uid}),buy:(id,qty)=>serverOnly('buy',{id,qty}),sell:(uid,qty)=>serverOnly('sell',{uid,qty}),warehouse:(action,uid,qty)=>serverOnly('warehouse',{action,uid,qty}),quest:(action,key)=>serverOnly('quest',{action,key}),chat:text=>serverOnly('world_chat',{text:String(text||'').slice(0,80)})};
function boot(){connect();setInterval(()=>{if(S.connected)send('ping',{t:Date.now()});},15000);}

...
// Death frames preload before timing starts, then play fall -> suspended half-dead -> full ground death.
(function(){
'use strict';
const FRAME_MS=115;
const HALF_HOLD_MS=720;
const GROUND_HOLD_MS=1800;
...
function loadDeathFrames(folder){
  if(deathFrames.has(folder))return Promise.resolve(deathFrames.get(folder));
  ...
  framePromises.set(folder,promise);
  return promise;
}
function ensureState(mob){
  let state=deathStates.get(mob.uid);
  if(state)return state;
  const folder=folderFor(mob.templateId);
  state={folder,frames:null,started:null,ready:false};
  deathStates.set(mob.uid,state);
  loadDeathFrames(folder).then(frames=>{
    const current=deathStates.get(mob.uid);
...
function prune(w){const current=new Set((w?.mobs||[]).map(m=>m.uid));for(const uid of [...deathStates.keys()])if(!current.has(uid))deathStates.delete(uid);}
function timeline(state,now){
  const frames=state.frames||[];
  const last=Math.max(0,frames.length-1);
  const mid=Math.max(0,Math.min(last-1,Math.round(last*0.55)));
  const firstCount=Math.max(1,mid+1);
  const secondCount=Math.max(0,last-mid);
  const fallDuration=firstCount*FRAME_MS;
  const finishDuration=secondCount*FRAME_MS;
  const elapsed=Math.max(0,now-state.started);
  if(elapsed<fallDuration)return {index:Math.min(mid,Math.floor(elapsed/FRAME_MS)),phase:'falling'};
  if(elapsed<fallDuration+HALF_HOLD_MS)return {index:mid,phase:'half-dead'};
  if(elapsed<fallDuration+HALF_HOLD_MS+finishDuration){
    const step=Math.floor((elapsed-fallDuration-HALF_HOLD_MS)/FRAME_MS)+1;
    return {index:Math.min(last,mid+step),phase:'finishing'};
  }
  return {index:last,phase:'dead-ground'};
}
function clearDeathCard(card,img,mob){
  ...
  if(img){delete img.dataset.deathLocked;img.style.removeProperty('transform');img.style.removeProperty('opacity');}
}
...
function renderLoop(){const snap=window.MMO149Realtime?.snapshot?.();if(snap?.world)renderDeathState(snap.world);}
...
function hook(){
  if(!window.ThinUI||window.ThinUI.__deathTimelineV2)return false;
  ...
  window.ThinUI.__deathTimelineV2=true;
  return true;
}
function injectStyles(){
  if(document.getElementById('death-timeline-v2-style'))return;
  const s=document.createElement('style');
  s.id='death-timeline-v2-style';
  s.textContent=`
#monster-death-layer{display:none!important}
#mob-list .mob-card.death-sequence{visibility:visible!important;opacity:1!important}
#mob-list .mob-card.death-sequence .mob-sprite{display:block!important;opacity:1!important;transition:none!important}
#mob-list .mob-card.death-sequence[data-death-phase="falling"] .mob-sprite,#mob-list .mob-card.death-sequence[data-death-phase="finishing"] .mob-sprite{transform:none!important}
#mob-list .mob-card.death-half .mob-sprite{transform:translateY(-4px)!important}
#mob-list .mob-card.death-ground .mob-sprite{transform:translateY(8px)!important}
.combat-kill{color:#fde68a;font-weight:700;margin-top:5px}.combat-reward{color:#86efac}.combat-drop{color:#93c5fd}
`;
  document.head.appendChild(s);
}
function boot(){document.getElementById('monster-death-layer')?.remove();injectStyles();if(!hook()){const retry=setInterval(()=>{if(hook())clearInterval(retry);},25);}setInterval(renderLoop,FRAME_TICK_MS);renderLoop();}

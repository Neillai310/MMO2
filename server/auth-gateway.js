...
const GAME_PORT=Number(process.env.GAME_PORT||PORT+1);
...
function hashPassword(p,s){return crypto.scryptSync(p,Buffer.from(s,'hex'),64).toString('hex')}
function makePassword(p){const salt=crypto.randomBytes(16).toString('hex');return{salt,hash:hashPassword(p,salt)}}
...
function handleMarket(client,p){
 const c=clientChar(client);if(!c)return;const action=cleanText(p.action,20);
 if(action==='list'){sendMarket(client);return;}
 if(action==='sell'){
  const it=findInv(c,cleanText(p.uid,40));if(!it)return error(client.ws,'找不到要上架的物品。');
  if(itemEquipped(c,it.uid))return error(client.ws,'裝備中的物品不能上架。');if(it.lock)return error(client.ws,'鎖定中的物品不能上架。');
  const qty=Math.max(1,Math.min(Number(it.cnt)||1,Math.floor(Number(p.qty)||1))),price=Math.max(1,Math.min(999999999,Math.floor(Number(p.price)||0)));
  if(marketListings.filter(x=>x.sellerToken===client.token).length>=20)return error(client.ws,'最多只能同時上架 20 筆。');
 ...
 if(action==='buy'){
  ...
  const x=marketListings[idx];if(x.sellerToken===client.token)return error(client.ws,'不能購買自己上架的商品。');if(c.gold<x.price)return error(client.ws,'金幣不足。');
  ...
  commit(client,{type:'market_buy',listingId:id,itemId:x.itemId,qty:x.qty,price:x.price,seller:x.sellerName});const sc=[...clients.values()].find(cl=>cl.token===x.sellerToken);if(sc)sendSnapshot(sc,{type:'market_sold',listingId:id,itemId:x.itemId,qty:x.qty,price:x.price,buyer:c.name});broadcastMarket();return;
 }
...
function settleOfflineAfk(c,token){
 const flags=ensureAfkFlags(c),since=Number(flags.offlineAfkSince)||0;flags.offlineAfkSince=0;if(!flags.offlineAfkEnabled||!since)return null;
 const seconds=Math.max(0,Math.min(8*3600,Math.floor((Date.now()-since)/1000))),map=MAPS[c.map];
 if(!map||map.safe||!Array.isArray(map.pool)||!map.pool.length)return {type:'offline_afk',seconds,kills:0,exp:0,gold:0,drops:[]};
 if(seconds<5)return {type:'offline_afk',seconds,kills:0,exp:0,gold:0,drops:[]};
 recalc(c);const interval=Math.max(6,14-Math.min(6,Math.floor((Number(c.lv)||1)/10))),kills=Math.min(4000,Math.floor(seconds/interval));let exp=0,gold=0;const totals=new Map();
 for(let i=0;i<kills;i++){const id=map.pool[rint(0,map.pool.length-1)],st=MOB_STATS[id]||[1,20,3,1],lv=Number(st[0])||1;exp+=Math.max(5,lv*7);gold+=rint(Math.max(1,lv*2),Math.max(3,lv*5));for(const d of rollDrops({templateId:id}))totals.set(d.id,(totals.get(d.id)||0)+d.cnt);}
 c.exp+=exp;c.gold+=gold;for(const [id,cnt]of totals)addItem(c,id,cnt);checkLevel(c);c.hp=c.maxHp;c.mp=c.maxMp;markDirty(token);
 return {type:'offline_afk',seconds,kills,exp,gold,drops:[...totals].map(([id,cnt])=>({id,cnt}))};
}
`;
 if(!src.includes('function handleMarket(client,p)')){
  if(!src.includes(marker))throw new Error('server.js is missing handleLogin; cannot build runtime safely');
...
let runtimeServer;
try{runtimeServer=buildRuntimeServer()}catch(e){console.error('Cannot prepare game runtime:',e);process.exit(1)}
const child=spawn(process.execPath,[runtimeServer],{cwd:__dirname,env:Object.assign({},process.env,{PORT:String(GAME_PORT),HOST:'127.0.0.1'}),stdio:'inherit'});
child.on('exit',(code,signal)=>{console.error(`Game server exited code=${code} signal=${signal||''}`);process.exit(code||1)});
...
function releaseSession(client){if(client.sessionKey&&activeSessions.get(client.sessionKey)===client)activeSessions.delete(client.sessionKey);client.sessionKey='';}
